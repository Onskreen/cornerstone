...
/**
 * Singleton class that carries out the animations and Surface operations in a
 * separate task on behalf of WindowManagerService.
 */
public class WindowAnimator {
	private static final String TAG = "WindowAnimator";

	/**
	 * Amount of time in milliseconds to animate the dim surface from one value
	 * to another, when no window animation is driving it.
	 */
	...
	/**
	 * Skip repeated AppWindowTokens initialization. Note that AppWindowsToken's
	 * version of this is a long initialized to Long.MIN_VALUE so that it
	 * doesn't match this value on startup.
	 */
	private int mAnimTransactionSequence;

	/**
	 * Window currently running an animation that has requested it be detached
	 * from the wallpaper. This means we need to ensure the wallpaper is visible
	 * behind it in case it animates in a way that would allow it to be seen. If
	 * multiple windows satisfy this, use the lowest window.
	 */
	...
	SparseArray<DisplayContentsAnimator> mDisplayContentsAnimators = new SparseArray<WindowAnimator.DisplayContentsAnimator>();
	...
	void removeDisplayLocked(final int displayId) {
		final DisplayContentsAnimator displayAnimator = mDisplayContentsAnimators
				.get(displayId);
	...
	AppWindowAnimator getWallpaperAppAnimator() {
		return mService.mWallpaperTarget == null ? null
				: mService.mWallpaperTarget.mAppToken == null ? null
						: mService.mWallpaperTarget.mAppToken.mAppAnimator;
	}
	...
	void hideWallpapersLocked(final WindowState w) {
		...
		if ((wallpaperTarget == w && lowerWallpaperTarget == null)
			...
			for (int i = numTokens - 1; i >= 0; i--) {
				...
				if (WindowManagerService.DEBUG_WALLPAPER_LIGHT && !token.hidden)
					Slog.d(TAG,
							"Hiding wallpaper " + token + " from " + w
									+ " target=" + wallpaperTarget + " lower="
									+ lowerWallpaperTarget + "\n"
									+ Debug.getCallers(5, "  "));
				token.hidden = true;
			}
		}
	...
	private void updateAppWindowsLocked() {
		...
		for (i = 0; i < NAT; i++) {
			...
			if (appAnimator.stepAnimationLocked(mCurrentTime)) {
				...
				if (WindowManagerService.DEBUG_ANIM)
					Slog.v(TAG, "updateWindowsApps...: done animating "
		...
		for (i = 0; i < NEAT; i++) {
			final AppWindowAnimator appAnimator = mService.mExitingAppTokens
					.get(i).mAppAnimator;
			final boolean wasAnimating = appAnimator.animation != null
					&& appAnimator.animation != AppWindowAnimator.sDummyAnimation;
			if (appAnimator.stepAnimationLocked(mCurrentTime)) {
				...
				if (WindowManagerService.DEBUG_ANIM)
					Slog.v(TAG, "updateWindowsApps...: done animating exiting "
							+ appAnimator.mAppToken);
			}
		}
	}
	...
	private void updateWindowsLocked(final int displayId) {
		...
		for (int i = windows.size() - 1; i >= 0; i--) {
			...
			if (winAnimator.mSurfaceControl != null) {
				...
				final boolean nowAnimating = winAnimator
						.stepAnimationLocked(mCurrentTime);
				...
				if (mPolicy.doesForceHide(win, win.mAttrs)) {
					if (!wasAnimating && nowAnimating) {
						if (WindowManagerService.DEBUG_ANIM
								|| WindowManagerService.DEBUG_VISIBILITY)
							Slog.v(TAG,
					...
					if (WindowManagerService.DEBUG_VISIBILITY)
						Slog.v(TAG, "Force hide " + mForceHiding
								+ " hasSurface=" + win.mHasSurface
								+ " policyVis=" + win.mPolicyVisibility
								+ " destroying=" + win.mDestroying
								+ " attHidden=" + win.mAttachedHidden + " vis="
								+ win.mViewVisibility + " hidden="
								+ win.mRootToken.hidden + " anim="
								+ win.mWinAnimator.mAnimation);
				} else if (mPolicy.canBeForceHidden(win, win.mAttrs)) {
					final boolean hideWhenLocked = (winAnimator.mAttrFlags & FLAG_SHOW_WHEN_LOCKED) == 0;
					final boolean changed;
					if (((mForceHiding == KEYGUARD_ANIMATING_IN) && (!winAnimator
						...
						if (changed) {
							...
							if (mCurrentFocus == null
									|| mCurrentFocus.mLayer < win.mLayer) {
		...
		if (unForceHiding != null) {
			for (int i = unForceHiding.size() - 1; i >= 0; i--) {
				Animation a = mPolicy
						.createForceHideEnterAnimation(wallpaperInUnForceHiding);
				if (a != null) {
					final WindowStateAnimator winAnimator = unForceHiding
							.get(i);
	...
	private void updateWallpaperLocked(int displayId) {
			...
			if (winAnimator.mAnimating) {
				if (winAnimator.mAnimation != null) {
					...
					if (backgroundColor != 0) {
						if (windowAnimationBackground == null
								|| (winAnimator.mAnimLayer < windowAnimationBackground.mAnimLayer)) {
		...
		if (mWindowDetachedWallpaper != detachedWallpaper) {
			if (WindowManagerService.DEBUG_WALLPAPER)
				Slog.v(TAG, "Detached wallpaper changed from "
						+ mWindowDetachedWallpaper + " to " + detachedWallpaper);
		...
		if (windowAnimationBackgroundColor != 0) {
			...
			if (mService.mWallpaperTarget == win
			 ...
			 * Date: 11/23/2012
			 *
			 * Sets the DimSurface width and height as per panel rect's
			 * width/height to restrict the DimSurface anim by not applying the
			 * screen width/height.
			 */
			final int dw = win.mFrame.width();
			final int dh = win.mFrame.height();
			displayAnimator.mWindowAnimationBackgroundSurface.mDimX = win.mFrame.left;
			displayAnimator.mWindowAnimationBackgroundSurface.mDimY = win.mFrame.top;
	...
	/**
	 * See if any windows have been drawn, so they (and others associated with
	 * them) can now be shown.
	 */
			...
			if (allDrawn != appAnimator.allDrawn) {
				...
				if (allDrawn) {
					...
					if (appAnimator.freezingScreen) {
						...
						mService.unsetAppFreezingScreenLocked(wtoken, false,
								true);
						if (WindowManagerService.DEBUG_ORIENTATION)
							Slog.i(TAG,
									"Setting mOrientationChangeComplete=true because wtoken "
											+ wtoken + " numInteresting="
											+ wtoken.numInterestingWindows
											+ " numDrawn="
											+ wtoken.numDrawnWindows);
						// This will set mOrientationChangeComplete and cause a
						// pass through layout.
						setAppLayoutChanges(
								appAnimator,
								WindowManagerPolicy.FINISH_LAYOUT_REDO_WALLPAPER,
	...
	private long getDimBehindFadeDuration(long duration) {
		...
		mContext.getResources().getValue(
				com.android.internal.R.fraction.config_dimBehindFadeDuration,
				tv, true);
	...
	private void animateLocked() {
		...
		try {
			...
			for (int i = 0; i < numDisplays; i++) {
				...
				DisplayContentsAnimator displayAnimator = mDisplayContentsAnimators
						.valueAt(i);
				 ...
				 */
				if (mService.mCornerstoneStateChangeAnimating) {
					for (int j = N - 1; j >= 0; j--) {
			...
			for (int i = 0; i < numDisplays; i++) {
				...
				if (winAnimator == null) {
					...
					dimLayer = winAnimator.mAnimLayer
							- WindowManagerService.LAYER_OFFSET_DIM;
					dimAmount = winAnimator.mWin.mAttrs.dimAmount;
				}

				if (dimAnimator != null && winAnimator != null) {
					/**
					 * Author: Onskreen
					 * Date: 13/08/2013
					 *
					 * Sets the DimAnimator's surface position and size as per
					 * WindowPanel's current position and size.
					 */
					WindowState w = winAnimator.mWin;
					if (w != null) {
						if (w.mAppToken != null) {
							WindowPanel wp = mService
									.findWindowPanel(w.mAppToken.token);
							if (wp != null) {
								Rect dimRect = wp.getPos();
								int dimWidth = dimRect.width();
								int dimHeight = dimRect.height();
								if (WindowManagerService.DEBUG_WINDOW_TRACE) {
									Slog.i(TAG, "dimWidth: " + dimWidth);
									Slog.i(TAG, "dimHeight: " + dimHeight);
									Slog.i(TAG, "wp: " + wp);
								}
								dimAnimator.mDimX = dimRect.left;
								dimAnimator.mDimY = dimRect.top;
								/**
								 * Author: Onskreen
								 * Date: 04/10/2013
								 *
								 * Sets the DimLayer rect width and height as
								 * per Panel's Rect.
								 */
								dimAnimator.mDimW = dimWidth;
								dimAnimator.mDimH = dimHeight;
							}
						}
					}
				}
				...
				if (targetAlpha != dimAmount) {
					if (winAnimator == null) {
						...
						long duration = (winAnimator.mAnimating && winAnimator.mAnimation != null) ? winAnimator.mAnimation
								.computeDurationHint() : DEFAULT_DIM_DURATION;
						if (targetAlpha > dimAmount) {
				 ...
				 * Date: 08/10/2013
				 *
				 * Sets the DimLayer rect's width and height
				 * as per Cornerstone Panel's current state.
				 */
				} else if (dimAnimator.mLastDimWidth != dimAnimator.mDimW
						|| dimAnimator.mLastDimHeight != dimAnimator.mDimH) {
					dimAnimator.show(dimLayer, targetAlpha,
				...
				// TODO (multidisplay): Magnification is supported only for the
				// default display.
				if (mService.mDisplayMagnifier != null
						&& displayId == Display.DEFAULT_DISPLAY) {
					mService.mDisplayMagnifier
		...
		for (int displayNdx = 0; displayNdx < numDisplays; ++displayNdx) {
			final DisplayContent displayContent = mService.mDisplayContents
					.valueAt(displayNdx);
			final int pendingChanges = getPendingLayoutChanges(displayContent
					.getDisplayId());
		...
		if (WindowManagerService.DEBUG_WINDOW_TRACE) {
			Slog.i(TAG,
					"!!! animate: exit mAnimating="
							...
							+ Integer
									.toHexString(getPendingLayoutChanges(Display.DEFAULT_DISPLAY)));
		}
	}
	...
	boolean isDimmingLocked(int displayId) {
		return getDisplayContentsAnimatorLocked(displayId).mDimAnimator
				.isDimming();
	}
	...
	public void dumpLocked(PrintWriter pw, String prefix, boolean dumpAll) {
		...
		for (int i = 0; i < mDisplayContentsAnimators.size(); i++) {
			...
			DisplayContentsAnimator displayAnimator = mDisplayContentsAnimators
					.valueAt(i);
			final WindowList windows = mService
					.getWindowListLocked(mDisplayContentsAnimators.keyAt(i));
			final int N = windows.size();
			for (int j = 0; j < N; j++) {
			...
			if (displayAnimator.mWindowAnimationBackgroundSurface != null) {
				if (dumpAll
					...
					displayAnimator.mWindowAnimationBackgroundSurface.printTo(
							subSubPrefix, pw);
				}
			}
			...
			if (displayAnimator.mScreenRotationAnimation != null) {
				...
				displayAnimator.mScreenRotationAnimation.printTo(subSubPrefix,
						pw);
			} else if (dumpAll) {
				pw.print(subPrefix);
	...
	void setAppLayoutChanges(final AppWindowAnimator appAnimator,
			final int changes, String s) {
	...
	void setDimWinAnimatorLocked(int displayId,
		...
		if (newWinAnimator == null) {
			...
			// Don't turn on for an unshown surface, or for any layer but the
			// highest dimmed layer.
			if (newWinAnimator.mSurfaceShown
					&& (existingDimWinAnimator == null
							|| !existingDimWinAnimator.mSurfaceShown || existingDimWinAnimator.mAnimLayer < newWinAnimator.mAnimLayer)) {
				displayAnimator.mDimWinAnimator = newWinAnimator;
			}
		}
	}

	private DisplayContentsAnimator getDisplayContentsAnimatorLocked(
			int displayId) {
		DisplayContentsAnimator displayAnimator = mDisplayContentsAnimators
				.get(displayId);
		if (displayAnimator == null) {
	...
	void setScreenRotationAnimationLocked(int displayId,
			ScreenRotationAnimation animation) {
	 ...
	 * Date: 13/08/2013
	 *
	 * Utility method returns the appropriate DimAnimator object as per diplay
	 * id.
	 */
	public DimLayer getDimLayer(int displayId) {
		final DisplayContentsAnimator displayAnimator = getDisplayContentsAnimatorLocked(displayId);
		if (displayAnimator != null) {
			return displayAnimator.mDimAnimator;
		}
		return null;
	}
	...
	private class DisplayContentsAnimator {
		...
		public DisplayContentsAnimator(int displayId) {
			...
			mWindowAnimationBackgroundSurface = new DimLayer(mService,
					displayId);
		}
	}
}